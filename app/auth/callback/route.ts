...
export async function GET(request: NextRequest) {
  ...
  const storedState = request.cookies.get("app2_oauth_state")?.value;
  ...
  try {
    ...
    if (!access.allowed) {
      ...
      res.cookies.delete("app2_oauth_state");
      return res;
    }
    ...
    const returnTo = request.cookies.get("app2_return_to")?.value ?? "/";
    const safeReturn =
    ...
    res.cookies.delete("app2_return_to");
    res.cookies.set("app2_session", session, {
      httpOnly: true,
      sameSite: "lax",
      path: "/",
      maxAge: 60 * 60 * 8,
